...
void main() {
  ...
  vec4 pos = gl_ProjectionMatrix * gl_ModelViewMatrix * gl_Vertex *
             vec4(cos(timeValX / 10.));

  // generate some noise values based on vertex position and the time value
  // which com/es in from our OF app
  float noiseAmntX = noise(vec2(-timeValX + pos.x / 1000.0f, 100.0f), 20.0);
  float noiseAmntY =
      noise(vec2(timeValY + pos.y / 1000.0f, pos.x / 200.0f), 20.0);

  // generate noise for our blue pixel value
  // float noiseB = noise( vec2(timeValY * 0.25, pos.y / 2000.0f), 20.0 );
  // float noiseB = cos(mouse.x/mouse.y);

  // lets also figure out the distance between the mouse and the vertex and
  // apply a repelling force away from the mouse
