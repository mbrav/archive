...
PRINT "=========================================".
PRINT "      Valentina v0.1".
PRINT "Preparing the launch of ship " + SHIP:NAME.
PRINT "Ship mass:    " + SHIP:MASS + "tons".
...
set rollAdjust to 90. // make the ship roll, so it wont pitch sideways
...
set prevmode to 0. // to track changes in runmode.
...
until runmode = 0 { //Run until we end the program
    ...
    if runmode = 1 { //Ship is on the launchpad
        ...
        if (SHIP:PERIAPSIS > targetPeriapsis) or (SHIP:PERIAPSIS > targetApoapsis * 0.95) or (SHIP:PERIAPSIS > Body("Kerbin"):ATM:HEIGHT) {
        ...
        lights on.
        unlock steering.
        HUDTEXT("SHIP SHOULD NOW BE IN SPACE!" + runmode + " Inititated", 3, 4, 15, green, true).
        set runmode to 0.
    }
    ...
    if prevmode <> runmode {
        HUDTEXT("Mode " + runmode + " Inititated", 3, 4, 15, green, true).
        set prevmode to runmode.
    }

    //Print data to screen.
    print "RUNMODE:    " + runmode + "      " at (5,1).
    print "ALTITUDE:   " + round(SHIP:ALTITUDE) + "      " at (5,2).
    print "APOAPSIS:   " + round(SHIP:APOAPSIS) + "      " at (5,3).
    print "PERIAPSIS:  " + round(SHIP:PERIAPSIS) + "      " at (5,4).
    print "ETA to AP:  " + round(ETA:APOAPSIS) + "      " at (5,5).
